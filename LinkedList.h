...
#define MAX_SIZE 10
// list의 최대 사이즈 저장. 가득 차있는지 안차있는지 확인용
...
template <class T> bool LinkedList<T>::Insert(T element) {
  if (current_size == MAX_SIZE) {
    cout << "error : trying to insert to full data set." << endl;
    return false;
  }
  // 노드 생성
  ...
  // 새 노드가 첫번째 노드를 가리킴
  // newnode는 포인터이기 때문에 -> 를 사용해 함수, 변수를 불러옴 newnode.link와
  // 뜻은 같다
  newnode->link = first;
  first = newnode;
  current_size++;
